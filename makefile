...
local-out-zip-file := MIUI_ace.zip

# the location for local-ota to save target-file
local-previous-target-dir := ~/workspace/ota_base/ace_4.1
...
local-miui-modified-apps := DeskClock MiuiHome Music Provision Settings Mms ThemeManager
...
local-pre-zip-misc:
	...
	cp other/bcmdhd.ko $(ZIP_DIR)/system/lib/modules/bcmdhd.ko
	cp other/keylayout/* $(ZIP_DIR)/system/usr/keylayout
	rm -rf $(ZIP_DIR)/system/lib/modules/scsi_wait_scan.ko
